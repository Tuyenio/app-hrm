...
class _SystemSettingsScreenState extends State<SystemSettingsScreen>
  ...
  final _addressCtrl = TextEditingController(
    text: 'Tầng 12, Tòa nhà ABC, Cầu Giấy, Hà Nội',
  );
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  'Quản lý cấu hình công ty & hệ thống',
          ...
          children: [
            Center(
              child: GestureDetector(
                onTap: () =>
                    showHrmSuccessSnackbar(context, 'Chọn logo công ty...'),
                child: Container(
                  width: 100,
                  height: 100,
                  decoration: BoxDecoration(
            ...
            _dropdownField('Ngành nghề', _sector, [
              'Công nghệ thông tin',
              'Tài chính',
              'Giáo dục',
              'Y tế',
              ...
              child: ElevatedButton.icon(
                onPressed: () {
                  if (_formKey.currentState!.validate())
                    showHrmSuccessSnackbar(context, 'Đã lưu thông tin công ty');
                },
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 4),
          Text(
            'Chọn gam màu phù hợp phong cách tổ chức',
            style: AppTextStyles.bodySmall.copyWith(
                ...
                child: InkWell(
                  ...
                  child: AnimatedContainer(
                    ...
                    decoration: BoxDecoration(
                      color: isSelected
                          ? (isDark
                                ? preset.primary.withValues(alpha: 0.12)
                                : preset.primarySurface)
                          : colors.surface,
                      borderRadius: BorderRadius.circular(14),
                      border: Border.all(
          ...
          Container(
            ...
            child: DropdownButtonHideUnderline(
              child: DropdownButton<String>(
                ...
                onChanged: (_) =>
                    showHrmSuccessSnackbar(context, 'Đã chọn ngôn ngữ'),
              ),
            ),
          ),
        ],
  ...
  Widget _buildTimekeepingTab(dynamic colors) {
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
          _buildConfigCard(
            'Tự động & Phương thức truy cập',
            Icons.settings_suggest_rounded,
            colors.primary,
            colors,
            [
              _toggleItem(
                'Tự động duyệt ca tăng nếu có chấm công',
                _autoApproveExtraShift,
                (v) => setState(() => _autoApproveExtraShift = v),
                colors,
              ),
              _toggleItem(
                'Cho phép chấm công qua WiFi công ty',
                _allowCompanyWifi,
                (v) => setState(() => _allowCompanyWifi = v),
                colors,
              ),
              _toggleItem(
                'Cho phép chấm công qua WiFi địa điểm',
                _allowLocationWifi,
                (v) => setState(() => _allowLocationWifi = v),
                colors,
              ),
              _toggleItem(
                'Bật chấm công bằng Face ID',
                _enableFaceId,
                (v) => setState(() => _enableFaceId = v),
                colors,
              ),
              _toggleItem(
                'Bật chấm công làm việc từ xa',
                _enableRemoteWork,
                (v) => setState(() => _enableRemoteWork = v),
                colors,
              ),
              _toggleItem(
                'Chỉ cho phép thiết bị đã đăng ký',
                _restrictRegisteredDevice,
                (v) => setState(() => _restrictRegisteredDevice = v),
                colors,
              ),
              _toggleItem(
                'Tự động chấm công ca liên tiếp',
                _autoCheckinConsecutive,
                (v) => setState(() => _autoCheckinConsecutive = v),
                colors,
              ),
              _toggleItem(
                'Yêu cầu hoàn thành task để check-out',
                _requireTaskCompletion,
                (v) => setState(() => _requireTaskCompletion = v),
                colors,
              ),
            ],
          ),
          ...
          _buildConfigCard(
            ...
            [
              _toggleItem(
                ...
                _alertCheckin,
                (v) => setState(() => _alertCheckin = v),
                colors,
              ),
              _toggleItem(
                'Gửi thông báo khi Check-out',
                _alertCheckout,
                (v) => setState(() => _alertCheckout = v),
                colors,
              ),
              _toggleItem(
                'Gửi thông báo khi đi muộn',
                _alertLate,
                (v) => setState(() => _alertLate = v),
                colors,
              ),
              _toggleItem(
                'Gửi thông báo khi về sớm',
                _alertEarly,
                (v) => setState(() => _alertEarly = v),
                colors,
              ),
            ],
          ),
          ...
          _buildConfigCard(
            ...
            [
              _radioGroup(
                'Logic đi muộn/về sớm (Part-time)',
                _partTimeLateLogic,
                [
                  ('shift', 'Tính từ giờ ca'),
                  ('actual', 'Tính từ giờ thực tế'),
                ],
                (v) => setState(() => _partTimeLateLogic = v!),
                colors,
              ),
              Divider(height: 20, color: colors.divider),
              _radioGroup(
                'Logic đi muộn/về sớm (Full-time)',
                _fullTimeLateLogic,
                [
                  ('shift', 'Tính từ giờ ca'),
                  ('actual', 'Tính từ giờ thực tế'),
                ],
                (v) => setState(() => _fullTimeLateLogic = v!),
                colors,
              ),
              Divider(height: 20, color: colors.divider),
              _radioGroup(
                'Logic chỉnh sửa của Quản lý',
                _managerEditLogic,
                [
                  ('manager', 'Dùng giờ Quản lý chỉnh'),
                  ('compare', 'So sánh với giờ ca'),
                ],
                (v) => setState(() => _managerEditLogic = v!),
                colors,
              ),
            ],
          ),
          const SizedBox(height: 12),
          _buildConfigCard(
            'Giới hạn GPS & Thời gian',
            Icons.gps_fixed_rounded,
            AppColors.error,
            colors,
            [
              _numericField(
                'Bán kính cảnh báo GPS (m)',
                _gpsWarningCtrl,
                'Mặc định: Diện tích x2',
              ),
              _numericField(
                'Bán kính GPS nghiêm ngặt (m)',
                _gpsStrictCtrl,
                null,
              ),
              _numericField(
                'Check-in sớm tối đa (phút)',
                _maxEarlyCheckinCtrl,
                null,
              ),
              _numericField(
                'Check-out muộn tối đa (phút)',
                _maxLateCheckoutCtrl,
                null,
              ),
              _numericField(
                'Khóa sửa sau payroll (ngày)',
                _lockPayrollCtrl,
                null,
              ),
              _numericField(
                'Khóa sửa sau cuối tuần (ngày)',
                _lockWeekCtrl,
                null,
              ),
              _numericField(
                'Giới hạn lần sửa cho quản lý',
                _maxEditCountCtrl,
                null,
              ),
            ],
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              onPressed: () =>
                  showHrmSuccessSnackbar(context, 'Đã lưu cấu hình chấm công'),
              icon: const Icon(Icons.save_rounded, size: 18),
